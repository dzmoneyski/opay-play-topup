-- تحسين دالة create_withdrawal لمعالجة أفضل للأخطاء
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE((setting_value->>'fixed_amount')::numeric, 0)
  ...
  -- ====== التحقق 8: كفاية الرصيد (قبل الخصم) ======
  IF _current_balance < _total_to_deduct THEN
    RETURN jsonb_build_object('success', false, 'error', 
      'رصيدك غير كافٍ. لديك ' || _current_balance || ' دج وتحتاج ' || _total_to_deduct || ' دج (المبلغ + الرسوم)');
  END IF;

  -- ====== خصم المبلغ + الرسوم (مع التأكد من عدم السالب) ======
  UPDATE user_balances
  SET balance = GREATEST(0, balance - _total_to_deduct),
      updated_at = now()
  WHERE user_id = _user_id
    AND balance >= _total_to_deduct;  -- شرط إضافي للأمان
  
  -- التحقق من نجاح الخصم
  IF NOT FOUND THEN
    RETURN jsonb_build_object('success', false, 'error', 
      'فشل في خصم المبلغ. تأكد من كفاية رصيدك.');
  END IF;
  ...
  -- ====== تسجيل الرسوم في platform_ledger ======
  IF _fee_amount > 0 THEN
    INSERT INTO platform_ledger (
      user_id, transaction_type, transaction_id, original_amount, 
      fee_amount, fee_percentage, fee_fixed
    ) VALUES (
      _user_id, 'withdrawal_fee', _withdrawal_id, _amount,
      _fee_amount, _fee_percentage, _fee_fixed
    );
  END IF;
...
EXCEPTION
  WHEN lock_not_available THEN
    RETURN jsonb_build_object('success', false, 'error', 'الطلب قيد المعالجة، يرجى الانتظار ثم المحاولة مرة أخرى');
  WHEN unique_violation THEN
    RETURN jsonb_build_object('success', false, 'error', 'رقم الحساب مستخدم في حساب آخر');
  WHEN check_violation THEN
    RETURN jsonb_build_object('success', false, 'error', 'رصيدك غير كافٍ لإتمام هذه العملية');
  WHEN OTHERS THEN
    -- إرجاع رسالة الخطأ الفعلية للتشخيص
    RETURN jsonb_build_object('success', false, 'error', 'خطأ: ' || SQLERRM);
END;
$$;