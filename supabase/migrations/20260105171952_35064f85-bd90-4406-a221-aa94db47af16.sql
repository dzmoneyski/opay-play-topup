...
-- Fix create_withdrawal to never violate user_balances.balance_non_negative constraint
-- and to return a clear "INSUFFICIENT_BALANCE" error instead of a Postgres constraint error.
-- Also align platform_ledger transaction_type with recalculate_user_balance (withdrawal_fee).
...
BEGIN
  ...
  -- ✅ Deduct balance safely (never go negative)
  UPDATE user_balances
  SET balance = balance - _total_to_deduct,
      updated_at = now()
  WHERE user_id = _user_id
    AND balance >= _total_to_deduct
  RETURNING balance INTO _new_balance;

  IF _new_balance IS NULL THEN
    -- Cleanup created withdrawal if deduction failed for any reason
    DELETE FROM withdrawals WHERE id = _withdrawal_id;
