...
-- حذف الدوال القديمة المتعارضة وإبقاء النسخة الجديدة فقط
DROP FUNCTION IF EXISTS public.process_digital_card_order(UUID, NUMERIC, TEXT);
DROP FUNCTION IF EXISTS public.process_digital_card_order(UUID, TEXT, NUMERIC);
...
BEGIN
  ...
  FROM (
    SELECT amount as spending FROM public.withdrawals
    WHERE user_id = _user_id AND status IN ('pending', 'completed') AND created_at >= CURRENT_DATE
    UNION ALL
    SELECT total_dzd as spending FROM public.digital_card_orders
    WHERE user_id = _user_id AND status IN ('pending', 'completed') AND created_at >= CURRENT_DATE
  ) combined_spending;
  ...
  IF _current_balance IS NULL THEN
    ...
    SELECT balance INTO _current_balance FROM public.user_balances WHERE user_id = _user_id;
  END IF;
  ...
  INSERT INTO public.digital_card_orders (
    ...
    _user_id, _card_type_id, TRIM(_account_id), _amount_dzd, _amount_usd,
    _card_type.exchange_rate, _fee_amount, _total_dzd, _total_dzd, 'pending'
  ) RETURNING id INTO _order_id;

  -- خصم الرصيد
  UPDATE public.user_balances SET balance = balance - _total_dzd, updated_at = now() WHERE user_id = _user_id;

  -- تسجيل رسوم المنصة
  INSERT INTO public.platform_ledger (user_id, transaction_type, transaction_id, original_amount, fee_amount, fee_percentage, currency)
  VALUES (_user_id, 'digital_card_fee', _order_id, _amount_dzd, _fee_amount,
    CASE WHEN _fee_settings IS NOT NULL AND _fee_settings.fee_type = 'percentage' THEN _fee_settings.fee_value ELSE _default_fee_percentage END, 'DZD');
