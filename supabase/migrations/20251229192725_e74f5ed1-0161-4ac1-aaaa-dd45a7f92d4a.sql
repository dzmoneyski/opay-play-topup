...
-- Update create_withdrawal function to add daily limit check
...
DECLARE
  ...
  _daily_limit numeric := 20000; -- حد السحب اليومي
...
BEGIN
  ...
  IF _today_total + _amount > _daily_limit THEN
    RETURN json_build_object(
      'success', false, 
      'error', 'عذراً، لقد وصلت للحد الأقصى اليومي للسحب (' || _daily_limit || ' دج). نتلقى العديد من طلبات السحب يومياً ونعمل جاهدين لتحسين خدماتنا. يمكنك المحاولة مرة أخرى غداً.',
