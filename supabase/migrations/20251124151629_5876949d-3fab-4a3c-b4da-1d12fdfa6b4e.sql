-- Update cancel_fraudulent_referral to mark suspicious_referrals as confirmed_fraud
...
BEGIN
  ...
  SELECT referrer_id, referred_user_id, reward_amount
  INTO _referrer_id, _referred_user_id, _reward_amount
  FROM public.referrals
  WHERE id = _referral_id AND status = 'active';
  ...
  -- Deduct reward from referrer's rewards balance
  ...
  _result := json_build_object(
    'success', true,
    'message', 'Referral cancelled and rewards deducted',
    'referrer_id', _referrer_id,
    'amount_deducted', _reward_amount
  );
