...
-- Fix recalculate_user_balance to deduct deposit fees from platform_ledger
...
BEGIN
  ...
  SELECT COALESCE(
    (
      ...
      -- - رسوم الإيداع من platform_ledger (المشكلة التي تم إصلاحها)
...
-- Recalculate balances for all users who have deposit fees
DO $$
DECLARE
  _user_record RECORD;
BEGIN
  FOR _user_record IN 
    SELECT DISTINCT user_id FROM platform_ledger WHERE transaction_type = 'deposit_fee'
  LOOP
