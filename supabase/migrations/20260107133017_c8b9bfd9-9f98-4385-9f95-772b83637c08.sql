...
-- إصلاح دالة حساب الرصيد لتشمل البطاقات المكتملة في الخصم
...
BEGIN
  ...
  -- ✅ Game topups (pending + approved + completed) - يُخصم المبلغ لجميع الحالات ما عدا rejected
  ...
  -- ✅ Phone topups (pending + approved + completed) - يُخصم المبلغ لجميع الحالات ما عدا rejected
  ...
  -- ✅ Digital cards (pending + approved + completed) - يُخصم المبلغ لجميع الحالات ما عدا rejected
  SELECT COALESCE(SUM(total_dzd), 0) INTO _total_digital_cards
  FROM public.digital_card_orders
  WHERE user_id = _user_id AND status IN ('pending', 'approved', 'completed');

  -- ✅ Betting deposits (pending + approved + completed)
