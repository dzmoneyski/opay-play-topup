-- إصلاح خطأ FOR UPDATE مع aggregate functions
...
BEGIN
  ...
  -- ====== التحقق 5: منع إنشاء طلب جديد إذا كان هناك طلب معلق (بدون FOR UPDATE مع aggregate) ======
  SELECT EXISTS(
  ...
  -- ====== التحقق 6: الحد اليومي (بدون FOR UPDATE) ======
  ...
  -- ====== التحقق 7: جلب إعدادات الرسوم ======
  SELECT 
    COALESCE((setting_value->>'fee_percentage')::numeric, 0),
    COALESCE((setting_value->>'fee_fixed')::numeric, 0)
  ...
  -- ====== حفظ حساب السحب للاستخدام المستقبلي ======
  IF _account_number IS NOT NULL AND TRIM(_account_number) != '' THEN
    ...
    ON CONFLICT (user_id, withdrawal_method, account_number) DO NOTHING;
  END IF;
