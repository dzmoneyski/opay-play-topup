...
BEGIN
  ...
  -- Get current balance with row lock (using FOR UPDATE without NOWAIT to wait instead of fail)
  ...
  -- Deduct balance - simplified without extra WHERE condition since we already locked the row
  ...
  IF _rows_updated = 0 THEN
    -- This should never happen since we already selected FOR UPDATE
    RAISE EXCEPTION 'فشل خصم الرصيد - خطأ في قاعدة البيانات';
  END IF;
